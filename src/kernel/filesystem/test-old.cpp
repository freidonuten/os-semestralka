...
int test2() {
	...
	std::shared_ptr<Path_Converter> path_converter = std::make_shared<Path_Converter>(element_factory);
	Element_And_Parent element_and_parent = path_converter->Get_Element_And_Parent_From_Path(root, "dir1/dir2/file1");

	char* data3 = new char[25];
	element_and_parent.element->Read(5000, 25, data3);
